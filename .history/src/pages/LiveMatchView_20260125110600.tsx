...
export default function LiveMatchView() {
  ...
  const isSpectator = !match || (
    !match.team1.players.some((p: any) => (p._id || p.id) === (user?._id || user?.id)) &&
    !match.team2.players.some((p: any) => (p._id || p.id) === (user?._id || user?.id))
  );

  const isCaptain = match && (
    (match.team1.captain._id || match.team1.captain.id) === (user?._id || user?.id) ||
    (match.team2.captain._id || match.team2.captain.id) === (user?._id || user?.id)
  );

  const confirmedFights = fights.filter(f => f.status === FightStatus.CONFIRMED);
  const team1Score = confirmedFights.filter(f => (f.winner?._id || f.winner?.id) && match?.team1.players.some((p: any) => (p._id || p.id) === (f.winner?._id || f.winner?.id))).length;
  const team2Score = confirmedFights.filter(f => (f.winner?._id || f.winner?.id) && match?.team2.players.some((p: any) => (p._id || p.id) === (f.winner?._id || f.winner?.id))).length;
        ...
        >
          {confirmedFights.length === 0 ? (
            ...
            confirmedFights.map((fight, index) => {
              const isTeam1Win = match.team1.players.some((p: any) => (p._id || p.id) === (fight.winner?._id || fight.winner?.id));
              return (
                <Box
