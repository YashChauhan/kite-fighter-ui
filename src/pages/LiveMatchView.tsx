...
import { getMatchById, confirmParticipation, declineParticipation, startMatch, declareWinner } from '../api/matches';
...
export default function LiveMatchView() {
  ...
  const [rosterMenuAnchor, setRosterMenuAnchor] = useState<{ element: HTMLElement; teamId: string } | null>(null);
  ...
  const canStartMatchValidation = () => {
    ...
    const team1Confirmed = match.team1?.players?.filter(
      (p: any) => p.confirmationStatus === 'confirmed'
    ).length || 0;
    
    const team2Confirmed = match.team2?.players?.filter(
      (p: any) => p.confirmationStatus === 'confirmed'
    ).length || 0;
    ...
    const now = new Date();
    if (matchDate > now) {
      const timeUntilMatch = Math.ceil((matchDate.getTime() - now.getTime()) / (1000 * 60)); // minutes
      return {
        allowed: true,
  ...
  const canStartMatch = match && (
    ...
    (Array.isArray(user?.clubs) && user.clubs.some((userClub: any) => {
      ...
      const isInvolvedClub = match.involvedClubs?.some((involvedClubId: string) => 
        involvedClubId === clubId
      ) || false;
  ...
  useEffect(() => {
    ...
    const loadMatch = async () => {
      try {
        ...
        console.log('📋 Team 1:', fetchedMatch.teams?.[0] || fetchedMatch.team1);
        console.log('📋 Team 2:', fetchedMatch.teams?.[1] || fetchedMatch.team2);
        ...
        console.log('📊 Fights response structure:', JSON.stringify(fightsResponse, null, 2));
        console.log('📊 Fights array:', fightsResponse.data);
        console.log('📊 Number of fights:', fightsResponse.data?.length);
        console.log('📊 Fights statuses:', fightsResponse.data?.map((f: any) => ({ id: f._id, status: f.status })));
        
        // Check if data is nested
        if (fightsResponse && Array.isArray(fightsResponse)) {
          console.log('⚠️ Fights response is an array directly, not wrapped in data object');
          ...
          console.error('❌ Unexpected fights response format:', fightsResponse);
          setFights([]);
        }
      } catch (err: any) {
  ...
  useEffect(() => {
      ...
      if (message.matchId === matchId && message.data?.fight) {
        ...
        setFights(prev => {
          const updated = (prev || []).map(f => 
            (f._id || f.id) === (message.data.fight._id || message.data.fight.id) 
              ? message.data.fight 
              : f
          );
    ...
    const unsubFightDisputed = socketService.onFightDisputed((message: any) => {
      if (message.matchId === matchId && message.data?.fight) {
        ...
        notificationService.warning('⚠️ Fight result disputed - requires admin review');
      }
    });
    ...
    const unsubPlayersRemoved = socketService.onPlayersRemoved((message: any) => {
      if (message.matchId === matchId || message.data?.matchId === matchId) {
    ...
    const unsubCaptainChanged = socketService.onCaptainChanged((message: any) => {
      if (message.matchId === matchId || message.data?.matchId === matchId) {
  ...
  const isSpectator = !match || (
    !match.team1?.players?.some((p: any) => {
      const playerId = p.playerId?._id || p.playerId?.id || p.playerId || p._id || p.id;
  ...
  const needsPlayerConfirmation = match && !isCaptain && match.status === MatchStatus.PENDING_PARTICIPANTS && (
    match.team1?.players?.some((p: any) => {
      const playerId = p.playerId?._id || p.playerId?.id || p.playerId || p._id || p.id;
      return playerId === (user?._id || user?.id) && p.confirmationStatus === 'pending';
    }) ||
    match.team2?.players?.some((p: any) => {
      const playerId = p.playerId?._id || p.playerId?.id || p.playerId || p._id || p.id;
      return playerId === (user?._id || user?.id) && p.confirmationStatus === 'pending';
  ...
  console.log('🔍 Confirmation Check:', {
    ...
    team1Captain: typeof match?.team1?.captain?.playerId === 'object' ? match?.team1?.captain?.playerId : { _id: match?.team1?.captain?.playerId },
    team1CaptainStatus: match?.team1?.captain?.confirmationStatus,
    team2Captain: typeof match?.team2?.captain?.playerId === 'object' ? match?.team2?.captain?.playerId : { _id: match?.team2?.captain?.playerId },
  ...
  const handleDeclineParticipation = async () => {
    ...
    try {
      setConfirmingParticipation(true);
      const response = await declineParticipation(matchId, playerId, reason || undefined);
      ...
      if (response.status === 'cancelled') {
        notificationService.warning('Match has been cancelled as no replacement captain was available');
      } else {
        notificationService.info('You have declined. A new captain has been assigned.');
      }
    } catch (err: any) {
  ...
  const handleStartMatch = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('organizer')) {
        notificationService.error('Only the match organizer can start this match');
      } else if (errorMessage.includes('participants')) {
        notificationService.error('Both teams need at least 2 confirmed players');
      } else if (errorMessage.includes('not found')) {
        notificationService.error('Match not found or is no longer available to start');
      } else {
  ...
  const handleFinishMatch = async () => {
    ...
    if (!isCaptain) {
      notificationService.error('Only team captains can declare the match winner');
    ...
    try {
      ...
      if (team1Score > team2Score) {
        ...
        notificationService.warning('Match is tied. Please select the winning team manually.');
        setShowFinishConfirm(false);
      ...
      if (updatedMatch.status === 'completed') {
        notificationService.success('🎉 Match completed! Both captains agreed on the winner.');
        ...
        notificationService.info('✅ Your declaration recorded. Waiting for other captain.');
      }
  ...
  const handleOpenRosterMenu = (event: React.MouseEvent<HTMLElement>, teamId: string) => {
  ...
  console.log('📊 All fights details:', fights?.map(f => ({ 
    id: f._id || f.id, 
    status: f.status, 
    result: f.result,
  ...
  console.log('📊 Fights by status:', {
    ...
    pending: fights?.filter(f => f.status === 'pending_captain_confirmation').length,
  ...
  return (
    <Box sx={{ pb: 10 }}>
      ...
      >
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <Box flex={1}>
            <Typography variant="h6">
              {match.team1?.club?.name || match.team1?.teamName || 'Team 1'} vs {match.team2?.club?.name || match.team2?.teamName || 'Team 2'}
            </Typography>
            <Typography variant="caption" color="text.secondary">
              {(match.scheduledAt || match.matchDate) ? format(new Date(match.scheduledAt || match.matchDate!), 'PPp') : 'Date not available'}
            </Typography>
          </Box>
        ...
        <Box display="flex" gap={1} mt={1}>
          <Chip 
            label={match.status} 
            color={
              match.status === MatchStatus.ACTIVE ? 'success' : 
              match.status === MatchStatus.READY_TO_START ? 'info' :
        ...
        <Box sx={{ p: 2 }}>
          ...
          >
            ...
            <Typography variant="caption">
              You have been selected as a captain for this match. Please confirm your participation.
            </Typography>
          </Alert>
        </Box>
      )}
      ...
      <Box sx={{ p: 2, textAlign: 'center', bgcolor: 'background.default' }}>
        ...
        <Box display="flex" gap={1} justifyContent="center" alignItems="center" flexWrap="wrap" mt={1}>
          <Typography variant="caption" color="text.secondary">
          ...
          {confirmedFights.length === 0 ? (
            ...
            confirmedFights.map((fight, index) => {
              ...
              const getPlayerName = (playerObj: any): string => {
                ...
                return player?.name || player?.playerName || playerObj.playerName || 'Unknown';
              };

              const team1PlayerName = getPlayerName(fight.team1Player || fight.player1);
              const team2PlayerName = getPlayerName(fight.team2Player || fight.player2);
              ...
              if (isDraw) {
                ...
                badgeColor = 'success.main'; // Green - user's team won
              } else if ((isUserOnTeam1 && team2Won) || (isUserOnTeam2 && team1Won)) {
      ...
      {(() => {
        ...
        const pendingFights = fights?.filter(f => f.status === 'pending_captain_confirmation') || [];
        ...
        const myPendingCount = pendingFights.filter(f => 
      ...
      {(() => {
        ...
        return (
          <Box sx={{ px: 2, py: 2, bgcolor: 'warning.50', borderLeft: '4px solid', borderColor: 'warning.main' }}>
            <Box display="flex" alignItems="center" gap={1} mb={2}>
              <TrophyIcon color="warning" />
              <Typography variant="subtitle1" fontWeight="bold" color="warning.dark">
                🏆 Match Result Declaration {currentRound > 1 && `(Round ${currentRound})`}
              </Typography>
            </Box>

            <Alert severity="info" sx={{ mb: 2 }}>
              <Typography variant="body2" fontWeight="medium">
                Other captain has declared <strong>{declaredWinnerName}</strong> as the winner.
              </Typography>
              <Typography variant="caption" color="text.secondary" display="block" mt={0.5}>
                Declared at: {new Date(otherDeclaration.declaredAt).toLocaleString()}
              </Typography>
            </Alert>
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    notificationService.error(err.response?.data?.message || 'Failed to approve');
                  } finally {
                    setFinishingMatch(false);
                  }
                }}
                ...
                onClick={async () => {
                  try {
                    ...
                    if (team1Score > team2Score) {
                      ...
                      myWinningTeamId = match.teams?.find(t => t.teamId !== otherDeclaration.declaredWinner)?.teamId || 'team2';
                    }
                    ...
                    notificationService.error(err.response?.data?.message || 'Failed to disagree');
                  } finally {
                    setFinishingMatch(false);
                  }
                }}
      ...
      <Box sx={{ p: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          <Box>
            <Typography variant="h6">
              {match.team1?.club?.name || (typeof match.team1?.clubId === 'object' ? (match.team1.clubId as any)?.name : '') || match.team1?.teamName || 'Team 1'}
            </Typography>
            <Typography variant="caption" color="text.secondary" display="block">
              Captain: {(typeof match.team1?.captain?.playerId === 'object' ? (match.team1.captain.playerId as any)?.name : '') || 'Not assigned'}
            </Typography>
          </Box>
          {canManageRosterState && navigator.onLine && (
            <IconButton
              onClick={(e) => handleOpenRosterMenu(e, match.teams?.[0]?.teamId || 'team1')}
              size="small"
              title="Manage Roster"
            >
              <ManageAccountsIcon />
        ...
        {match.team1?.players && match.team1.players.length > 0 ? (
          match.team1.players.map((playerData: any) => {
            ...
            const playerName = player?.name || playerData.playerName || 'Unknown';
                ...
                <Box flex={1}>
                  <Box display="flex" alignItems="center" gap={1}>
                    ...
                    <Box display="flex" gap={0.5}>
                      {confirmedFights
                        .filter(fight => {
                          const player1 = fight.team1Player || fight.player1;
                          const player2 = fight.team2Player || fight.player2;
                          const p1Id = player1?.playerId?._id || player1?.playerId?.id || player1?._id || player1?.id;
                          const p2Id = player2?.playerId?._id || player2?.playerId?.id || player2?._id || player2?.id;
                        ...
                        .map((fight, idx) => {
                          ...
                          const player1 = fight.team1Player || fight.player1;
                          const p1Id = player1?.playerId?._id || player1?.playerId?.id || player1?._id || player1?.id;
                          const playerWon = (fightResult === 'player1' && p1Id === playerId) || 
                                           (fightResult === 'player2' && p1Id !== playerId);
                          const isDraw = fightResult === 'draw';
                          ...
                          return (
                            <Box
                              ...
                              sx={{
                                ...
                                height: 12,
                                borderRadius: '50%',
                                bgcolor: isDraw ? 'warning.main' : playerWon ? 'success.main' : 'error.main',
                              }}
                            />
                          );
                        })}
      ...
      <Box sx={{ p: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
          <Box>
            <Typography variant="h6">
              {match.team2?.club?.name || (typeof match.team2?.clubId === 'object' ? (match.team2.clubId as any)?.name : '') || match.team2?.teamName || 'Team 2'}
            </Typography>
            <Typography variant="caption" color="text.secondary" display="block">
              Captain: {(typeof match.team2?.captain?.playerId === 'object' ? (match.team2.captain.playerId as any)?.name : '') || 'Not assigned'}
            </Typography>
          </Box>
          {canManageRosterState && navigator.onLine && (
            <IconButton
              onClick={(e) => handleOpenRosterMenu(e, match.teams?.[1]?.teamId || 'team2')}
              size="small"
              title="Manage Roster"
            >
              <ManageAccountsIcon />
      ...
      {canStartMatch && (match.status === MatchStatus.READY_TO_START || match.status === 'ready_to_start' || match.status === 'ready' || match.status === 'pending_participants') && (() => {
        const validation = canStartMatchValidation();
        return (
          <Box
            sx={{
          ...
          >
            <Button
              ...
              title={validation.allowed ? (validation.warning || 'Click to start the match') : validation.reason}
              sx={{ height: 48 }}
            >
              {startingMatch ? 'Starting Match...' : validation.allowed ? 'Start Match' : 'Cannot Start'}
            </Button>
          ...
          >
            <CardContent>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                {canStartMatchValidation().warning || 'Are you sure you want to start this match?'}
              </Typography>
        ...
        >
          ...
          >
            <CardContent>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                Are you sure you want to finish this match? The winner will be determined based on the current score.
              </Typography>
              <Box display="flex" gap={1} justifyContent="flex-end">
                <Button
                  variant="outlined"
      ...
      >
        <MenuItem onClick={() => rosterMenuAnchor && handleAddPlayers(rosterMenuAnchor.teamId)}>
          <ListItemIcon>
            <PersonAddIcon fontSize="small" />
          </ListItemIcon>
          <ListItemText>Add Players</ListItemText>
        </MenuItem>
        <MenuItem onClick={() => rosterMenuAnchor && handleRemovePlayers(rosterMenuAnchor.teamId)}>
          <ListItemIcon>
            <PersonRemoveIcon fontSize="small" />
          </ListItemIcon>
          <ListItemText>Remove Players</ListItemText>
        </MenuItem>
        <MenuItem onClick={() => rosterMenuAnchor && handleChangeCaptain(rosterMenuAnchor.teamId)}>
          <ListItemIcon>
